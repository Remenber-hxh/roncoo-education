 ...
 * 学习统计看板数据（二开）
 * <p>
 * 完成度按 user_study 的实际进度与 exam_record 的通过记录现算，
 * 不读 user_course_assign.finish_status——那是个冗余状态，
 * 历史上只有「考试通过」一条路会写它，照它统计会把已经学完的人算成未开始。
 * <p>
 * 「完成」= 课时全部学完 <b>或</b> 该课程考试已通过。漏掉后半条的话，
 * 考了满分但没把每个课时都点开的员工会被算成未完成、进逾期名单、还收到催办。
 *
 * @author 二开
 */
@Data
